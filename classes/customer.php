	...
	class customer
	{
		...
		public function insert_binhluan(){
			...
			if($tenbinhluan=='' || $binhluan==''){
				$alert ="<span class='error'> Trường này không này để trống </span>";
				return $alert;
			}else{
				$query = "INSERT INTO tbl_binhluan(tenbinhluan,binhluan,product_id) VALUES('$tenbinhluan','$binhluan','$product_id')";
					$result = $this->db->insert($query);
					if($result){
						$alert = "<span class='success'>Bình luận thành công !, bình luận này sẽ được admin kiểm duyệt trong vòng 24 giờ</span>";
						return $alert;
					}else{
